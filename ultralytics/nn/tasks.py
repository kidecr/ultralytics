...
from ultralytics.utils.loss import v8ClassificationLoss, v8DetectionLoss, v8OBBLoss, v8PoseLoss, v8SegmentationLoss, RGBIRLoss
...
class BaseModel(nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        """
        model = weights["model"] if isinstance(weights, dict) and weights.has_key("model") else weights  # torchvision models are not dicts
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Define model
        self.ch = self.yaml["ch"] = self.yaml.get("ch", ch)  # input channels # 设置输入通道大小
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            c1, c2 = ch[f], args[0] # c1:该层输入通道大小，为上一层输出大小， c2为该层输出大小，由第一个参数给出
            ...
            args = [c1, c2, *args[1:]] # 参数为输入层大小，输出层大小，配置文件中从第二个开始的其他参数
